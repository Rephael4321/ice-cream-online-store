...
| **withMiddleware(..., { middleware: validateClientOrderAccess })** | After normal auth: **admin-equivalent** → redirect to CMS; **client** → must have cookie `phoneNumber` and order must belong to that phone. |
...
| POST | `/api/orders` | **Anyone** | **skipAuth** – create order (checkout). Body: `phone`, `items`, optional `isNotified`. After successful DB commit, server **fire-and-forget** sends Web Push to all stored subscriptions for users with role `admin`, `superuser`, or `driver` (no-op if VAPID env missing or no rows). | Yes (cart checkout) |
...
| POST | `/api/push/test` | **Admin**, **Superuser**, **Driver** | Send a test payload to current user’s subscriptions only. | CMS `/notifications` |
