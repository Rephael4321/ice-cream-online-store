...
export default function OrderDetails() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <Link href="/cms/orders" className="text-blue-600 hover:underline">
        ← חזרה לרשימת הזמנות
      </Link>
      ...
      <div className="border p-4 rounded shadow">
        ...
        <ul className="space-y-4">
          {items.map((item, i) => {
            ...
            if (hasSale) {
              bundles = Math.floor(item.quantity / item.saleQuantity!);
              remainder = item.quantity % item.saleQuantity!;
              finalTotal =
